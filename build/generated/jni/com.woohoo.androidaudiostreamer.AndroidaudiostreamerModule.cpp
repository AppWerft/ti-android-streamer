...
, javaClass, nameSymbol);
	...
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "getDeclaredBitRate", AndroidaudiostreamerModule::getDeclaredBitRate);
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "setAllowBackground", AndroidaudiostreamerModule::setAllowBackground);
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "setDecodeBufferCapacityMs", AndroidaudiostreamerModule::setDecodeBufferCapacityMs);
	...
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "setAudioBufferCapacityMs", AndroidaudiostreamerModule::setAudioBufferCapacityMs);
	...
	if (!env) {
		...
		DEFINE_INT_CONSTANT(prototypeTemplate, "STATE_BUFFERING", 1);
...
Handle<Value> AndroidaudiostreamerModule::getDeclaredBitRate(const Arguments& args)
{
	LOGD(TAG, "getDeclaredBitRate()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(AndroidaudiostreamerModule::javaClass, "getDeclaredBitRate", "()I");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getDeclaredBitRate' with signature '()I'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> AndroidaudiostreamerModule::setDecodeBufferCapacityMs(const Arguments& args)
{
	LOGD(TAG, "setDecodeBufferCapacityMs()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(AndroidaudiostreamerModule::javaClass, "setDecodeBufferCapacityMs", "(I)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'setDecodeBufferCapacityMs' with signature '(I)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
	...
	if (args.Length() < 1) {
		char errorStringBuffer[100];
		sprintf(errorStringBuffer, "setDecodeBufferCapacityMs: Invalid number of arguments. Expected 1 but got %d", args.Length());
		return ThrowException(Exception::Error(String::New(errorStringBuffer)));
	}
...
Handle<Value> AndroidaudiostreamerModule::setAudioBufferCapacityMs(const Arguments& args)
{
	LOGD(TAG, "setAudioBufferCapacityMs()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(AndroidaudiostreamerModule::javaClass, "setAudioBufferCapacityMs", "(I)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'setAudioBufferCapacityMs' with signature '(I)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
	...
	if (args.Length() < 1) {
		char errorStringBuffer[100];
		sprintf(errorStringBuffer, "setAudioBufferCapacityMs: Invalid number of arguments. Expected 1 but got %d", args.Length());
		return ThrowException(Exception::Error(String::New(errorStringBuffer)));
	}
