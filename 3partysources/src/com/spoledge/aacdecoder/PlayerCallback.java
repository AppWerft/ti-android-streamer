...
public interface PlayerCallback {
	 ...
	 * 
	 * @param perf
	 *            performance indicator - how much is decoder faster than audio
	 *            player in %; if less than 0, then decoding of audio is slower
	 *            than needed by audio player; example: perf = 53 means that
	 *            audio decoder is 53% faster than audio player (production of
	 *            audio data is 1.53 faster than consumption of audio data)
	 */
	...
	/**
	 * This method is called when the player receives a metadata information. It
	 * can be either before starting the player (from HTTP header - all header
	 * pairs) or during playback (metadata frame info).
	 *
	 * The list of available keys is not part of this project - it is depending
	 * on the server implementation.
	 *
	 * @param key
	 *            the metadata key - e.g. from HTTP header: "icy-genre",
	 *            "icy-url", "content-type",.. or from the dynamic metadata
	 *            frame: e.g. "StreamTitle" or "StreamUrl"
	 * @param value
	 *            the metadata value
	 */
