...
/**
 * This is the Multi (MP3/AAC) Stream player class. It uses Decoder to decode
 * Multi stream into PCM samples. This class is not thread safe.
 * 
 * <pre>
...
public class MultiPlayer extends AACPlayer {
	...
	protected void processHeaders(URLConnection cn) {
		...
		for (java.util.Map.Entry<String, java.util.List<String>> me : cn
			...
			if ("content-type".equalsIgnoreCase(me.getKey())) {
				for (String s : me.getValue()) {
					...
					// audio/x-mpegaudio
					boolean isMp3 = s.startsWith("mp3") || s.startsWith("mpeg")
							|| s.startsWith("mpg");

					Log.i(LOG, "Setting " + (isMp3 ? "MP3" : "AAC")
							+ " decoder for content type " + ct);
		...
		throw new RuntimeException(
				"Could not recognize the type of the stream.");
	}

	/**
	 * This method is called before opening the file. Detects the file type -
	 * according to the suffix.
	 */
	@Override
	protected void processFileType(String file) {
		boolean isMp3 = file.toLowerCase().endsWith(".mp3");
		Log.i(LOG, "Setting " + (isMp3 ? "MP3" : "AAC") + " decoder for file "
				+ file);
		setDecoder(isMp3 ? mp3Decoder : aacDecoder);
	}
