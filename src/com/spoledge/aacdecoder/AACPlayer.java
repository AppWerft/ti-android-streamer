...
/**
 * This is the AAC Stream player class. It uses Decoder to decode AAC stream
 * into PCM samples. This class is not thread safe.
 * 
 * <pre>
...
public class AACPlayer {
	...
	/**
	 * The default expected bitrate. Used only if not specified in play()
	 * methods.
	 */
	public static final int DEFAULT_EXPECTED_KBITSEC_RATE = 128;
	...
	public AACPlayer(PlayerCallback playerCallback) {
		this(playerCallback, DEFAULT_AUDIO_BUFFER_CAPACITY_MS,
				DEFAULT_DECODE_BUFFER_CAPACITY_MS);
	}
	 ...
	 *            the capacity of the audio buffer (AudioTrack) in ms
	 * @param decodeBufferCapacityMs
	 *            the capacity of the buffer used for decoding in ms
	 * @see setAudioBufferCapacityMs(int)
	 * @see setDecodeBufferCapacityMs(int)
	 */
	public AACPlayer(PlayerCallback playerCallback, int audioBufferCapacityMs,
			int decodeBufferCapacityMs) {
	...
	/**
	 * Sets the audio buffer (AudioTrack) capacity. The capacity can be
	 * expressed in time of audio playing of such buffer. For example 1 second
	 * buffer capacity is 88100 samples for 44kHz stereo. By setting this the
	 * audio will start playing after the audio buffer is first filled.
	 *
	 * NOTE: this should be set BEFORE any of the play methods are called.
	 *
	 * @param audioBufferCapacityMs
	 *            the capacity of the buffer in milliseconds
	 */
	...
	/**
	 * Sets the capacity of the output buffer used for decoding. The capacity
	 * can be expressed in time of audio playing of such buffer. For example 1
	 * second buffer capacity is 88100 samples for 44kHz stereo. Decoder tries
	 * to fill out the whole buffer in each round.
	 *
	 * NOTE: this should be set BEFORE any of the play methods are called.
	 *
	 * @param decodeBufferCapacityMs
	...
	/**
	 * Gets the capacity of the output buffer used for decoding as the audio
	 * playing time.
	 * 
	...
	/**
	 * Sets the PlayerCallback. NOTE: this should be set BEFORE any of the play
	 * methods are called.
	 */
	...
	/**
	 * Returns the flag if metadata information is enabeld / sent to
	 * PlayerCallback.
	 */
	...
	/**
	 * Sets the flag if metadata information is enabeld / sent to
	 * PlayerCallback. This is enabled by default.
	 */
	...
	/**
	 * Returns the flag if the HTTP / shoutcast response code should be checked
	 * or not.
	 */
	...
	/**
	 * Sets the flag if the HTTP / shoutcast response code should be checked or
	 * not. This method was added for backward compatibility. By disabling the
	 * check you also force pre-Kitkat devices to use original HttpURLConnection
	 * implementation even for shoutcast streams. This is enabled by default.
	 * 
	 * @since 0.8
	 */
	...
	/**
	 * Sets the encoding for the metadata strings. If not set, then UTF-8 is
	 * used.
	 */
	...
	/**
	 * Plays a stream asynchronously. This method starts a new thread.
	 * 
	 * @param url
	 *            the URL of the stream or file
	 */
	 ...
	 * @param url
	 *            the URL of the stream or file
	 * @param expectedKBitSecRate
	 *            the expected average bitrate in kbit/sec; -1 means unknown
	 */
	 ...
	 * @param expectedKBitSecRate
	 *            the expected average bitrate in kbit/sec; -1 means unknown;
	 *            when setting this parameter, then the declared bit-rate from
	 *            the stream header is ignored
	 */
		...
		if (url.indexOf(':') > 0) {
			...
			try {
				if (responseCodeCheckEnabled)
					checkResponseCode(cn);
				processHeaders(cn);
				is = getInputStream(cn);

				// try to get the expectedKBitSecRate from headers
				// but if then expectedKBitSecRate is passed, then ignore the
				// declared one:
				play(is, expectedKBitSecRate != -1 ? expectedKBitSecRate
						: declaredBitRate);
			} finally {
				try {
					is.close();
	 ...
	 *            the expected average bitrate in kbit/sec; -1 means unknown
	 */
	public final void play(InputStream is, int expectedKBitSecRate)
			throws Exception {
		stopped = false;
	...
	/**
	 * Plays a stream synchronously. This is the implementation method calle by
	 * every play() and playAsync() methods.
	 * 
	 * @param is
	 *            the input stream
	 * @param expectedKBitSecRate
	 *            the expected average bitrate in kbit/sec
	 */
	protected void playImpl(InputStream is, int expectedKBitSecRate)
			throws Exception {
		BufferReader reader = new BufferReader(computeInputBufferSize(
				expectedKBitSecRate, decodeBufferCapacityMs), is);
		...
		try {
			...
			Log.d(LOG, "play(): samplerate=" + info.getSampleRate()
					+ ", channels=" + info.getChannels());

			profSampleRate = info.getSampleRate() * info.getChannels();

			if (info.getChannels() > 2) {
				throw new RuntimeException("Too many channels detected: "
						+ info.getChannels());
			}
			...
			do {
				...
				if (nsamp == 0 || stopped)
					break;
				if (!pcmfeed.feed(decodeBuffer, nsamp) || stopped)
					break;

				int kBitSecRate = computeAvgKBitSecRate(info);
				if (Math.abs(expectedKBitSecRate - kBitSecRate) > 1) {
					Log.i(LOG, "play(): changing kBitSecRate: "
							+ expectedKBitSecRate + " -> " + kBitSecRate);
					reader.setCapacity(computeInputBufferSize(kBitSecRate,
							decodeBufferCapacityMs));
					expectedKBitSecRate = kBitSecRate;
				}
			...
			if (profCount > 0)
				Log.i(LOG, "play(): average decoding time: " + profMs
						/ profCount + " ms");
			...
			if (profMs > 0) {
				...
				Log.i(LOG,
						"play(): average rate (samples/sec): audio="
								+ profSampleRate
								+ ", decoding="
								+ (1000 * profSamples / profMs)
								+ ", audio/decoding= "
								+ perf
	...
	protected short[][] createDecodeBuffers(int count, Decoder.Info info) {
		int size = PCMFeed.msToSamples(decodeBufferCapacityMs,
	...
	protected PCMFeed createPCMFeed(Decoder.Info info) {
		int size = PCMFeed.msToBytes(audioBufferCapacityMs,
				info.getSampleRate(), info.getChannels());

		return new PCMFeed(info.getSampleRate(), info.getChannels(), size,
				playerCallback);
	}

	/**
	 * Opens connection. Tries to recognize if the stream is a standard HTTP or
	 * SHOUTCAST. Since Android 4.4 Kitkat the HttpURLConnection implementation
	 * is strict and does not allow SHOUTCAST response "ICY 200 OK". If we
	 * detect this, we try to use alternate protocol "icy" and our auxiliar
	 * implementation - IcyURLConnection. NOTE: URL.setURLStreamHandlerFactory()
	 * must be called - this library does not call it itself.
	 */
			...
			try {
				if (conn instanceof HttpURLConnection) {
					...
					try {
						...
						if (httpConn.getResponseCode() == -1) {
							if (!responseCodeCheckEnabled) {
								Log.w(LOG,
										"No response code, but ignoring - for url "
												+ url);
								close = false;
								break;
							} else {
							...
							// standard HTTP response / IcyURLConnection
							// response
							close = false;
							break;
						}
						...
						// ICY 200 OK
						Log.w(LOG, "Invalid response code for url " + url
								+ " - " + e);
					}
				...
				if (url.startsWith("http:")) {
					...
					throw new IOException(
							"Invalid response - no response code / headers detected");
			} finally {
				if (close) {
					if (conn instanceof HttpURLConnection) {
						try {
	...
	/**
	 * Prepares the connection. This method is called before a connection is
	 * opened. Actually sets "Icy-MetaData" header to "1" if metadata are
	 * enabled.
	 */
	...
	/**
	 * Checks the response code. Actually for HttpURLConnection it throws an
	 * exception when the response code is not between 200 and 299.
	 */
	protected void checkResponseCode(URLConnection conn) throws Exception {
		if (conn instanceof HttpURLConnection) {
			...
			if (responseCode == -1) {
				Log.w(LOG, "Empty response code: " + responseCode + " "
						+ httpConn.getResponseMessage());
			} else if (responseCode < 200 || responseCode > 299) {
				Log.e(LOG, "Error response code: " + responseCode + " "
						+ httpConn.getResponseMessage());
	...
	/**
	 * Gets the input stream from the connection. Actually returns the
	 * underlying stream or IcyInputStream.
	 */
	protected InputStream getInputStream(URLConnection conn) throws Exception {
		...
		if (!metadataEnabled) {
			...
			try {
				...
				Log.e(LOG, "The icy-metaint '" + smetaint
						+ "' cannot be parsed: '" + e);
			}

			if (period > 0) {
				Log.i(LOG, "The dynamic metainfo is sent every " + period
						+ " bytes");
	...
	protected void processHeaders(URLConnection cn) {
		...
		if (br != null) {
			try {
				...
				if (declaredBitRate > 7) {
					Log.d(LOG, "Declared bitrate is " + declaredBitRate
							+ " kb/s");
				} else {
					Log.w(LOG, "Declared bitrate is too low - ignoring: "
							+ declaredBitRate + " kb/s");
					declaredBitRate = -1;
				}
			} catch (Exception e) {
		...
		if (playerCallback != null) {
			for (java.util.Map.Entry<String, java.util.List<String>> me : cn
	...
	/**
	 * This method is called before opening the file. Actually this method does
	 * nothing, but subclasses may override it.
	 */
	protected void processFileType(String file) {
	}

	protected int computeAvgKBitSecRate(Decoder.Info info) {
		// do not change the value after a while - avoid changing of the out
		// buffer:
	...
	protected static int computeKBitSecRate(Decoder.Info info) {
		...
		return computeKBitSecRate(info.getRoundBytesConsumed(),
				info.getRoundSamples(), info.getSampleRate(),
				info.getChannels());
	}

	protected static int computeKBitSecRate(int bytesconsumed, int samples,
	...
	protected static int computeInputBufferSize(Decoder.Info info,
			int durationMs) {

		return computeInputBufferSize(info.getRoundBytesConsumed(),
				info.getRoundSamples(), info.getSampleRate(),
				info.getChannels(), durationMs);
	}

	protected static int computeInputBufferSize(int bytesconsumed, int samples,
			int sampleRate, int channels, int durationMs) {

		return (int) (((long) bytesconsumed) * channels * sampleRate
				* durationMs / (1000L * samples));
	}
